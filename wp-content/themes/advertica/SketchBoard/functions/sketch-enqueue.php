...
function advertica_script_enqueqe() {
	global $advertica_shortname;
	if(!is_admin()) {
		wp_enqueue_script('advertica_flexslider_js', get_template_directory_uri() .'/js/jquery.flexslider-min.js',array('jquery'),'1.0',1 );
...
function advertica_admin_script_enqueqe(){
	if(is_admin()){
		wp_enqueue_script('jquery-ui-core');
		wp_enqueue_script('jquery-ui-sortable');
	}
}
add_action('admin_init', 'advertica_admin_script_enqueqe');
...
function advertica_theme_stylesheet()
{
	...
	{
		$theme = wp_get_theme();
		wp_enqueue_script('advertica_colorboxsimple_slide',get_template_directory_uri() .'/js/jquery.prettyPhoto.js',array('jquery'),true,'1.0');
		...
		wp_enqueue_script('advertica_isotope_slide',get_template_directory_uri().'/js/isotope.js',array('jquery'),'1.0',true);
		...
		wp_enqueue_script('adverticat_nicescroll',get_template_directory_uri().'/js/jquery.nicescroll.min.js',array('jquery'),'1.0',true );
		...
		wp_enqueue_style('advertica-flexslider-stylesheet', get_template_directory_uri().'/css/flexslider.css', false, $theme->Version);
...
function advertica_head() {
	...
	/***************** Custom CSS ****************/
	$skt_custom_css = sketch_get_option($advertica_shortname.'_custom_css');
