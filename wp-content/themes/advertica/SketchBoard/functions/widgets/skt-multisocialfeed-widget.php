...
class SktMultiSocialFeed extends WP_Widget {
    ...
    function SktMultiSocialFeed() {
		...
		$this->WP_Widget('SktMultiSocialFeed',"MultiSocialFeed - $advertica_themename", $widget_ops);	
    }
    ...
    function form($instance) {
		...
		if(empty($social_network)) {$social_network= __('pinterest','advertica');}
        ?>
         <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:','advertica'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php if(isset($title)){echo $title;} else { echo 'MultiSocial';}  ?>" /></label></p>
         <p><label for="<?php echo $this->get_field_id('user_name'); ?>"><?php _e('Username:','advertica'); ?> <input class="widefat" id="<?php echo $this->get_field_id('user_name'); ?>" name="<?php echo $this->get_field_name('user_name'); ?>" type="text" value="<?php if(isset($user_name)){echo $user_name;} else { echo 'sketchthemes';} ?>" /></label></p>
         <p><label for="<?php echo $this->get_field_id('num_posts'); ?>"><?php _e('Number Of Post: eg:4','advertica'); ?> <input class="widefat" id="<?php echo $this->get_field_id('num_posts'); ?>" name="<?php echo $this->get_field_name('num_posts'); ?>" type="text" value="<?php if(isset($num_posts)){echo $num_posts;} ?>" /></label></p>		 		
		 <p>
			 <label for="<?php echo $this->get_field_id('social_network'); ?>"><?php _e('Select Network:','advertica'); ?>
				 <select class="widefat" id="<?php echo $this->get_field_id('social_network'); ?>" name="<?php echo $this->get_field_name('social_network'); ?>">
					 <option value="pinterest" <?php selected('pinterest', $social_network); ?>><?php _e('Pinterest','advertica');?></option>
					 <option value="dribbble" <?php selected('dribbble', $social_network); ?>><?php _e('Dribbble','advertica');?></option>
					 <option value="flickr" <?php selected('flickr', $social_network); ?>><?php _e('Flickr','advertica');?></option>
					 <option value="instagram" <?php selected('instagram', $social_network); ?>><?php _e('Instagram','advertica');?></option>
				 </select>
			 </label>
		</p>
        <?php 
