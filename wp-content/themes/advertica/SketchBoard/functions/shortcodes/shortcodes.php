...
function skt_shortcodes_css(){
	...
	wp_enqueue_script('skt-dynamo-js', SKT_SHORTCODES_DIR . '/js/dynamo.min.js', array('jquery'), SKT_SHORTCODES_VERSION, true);
}
...
function skt_youtube($atts, $content=null){  
	...
	$video_id = $output[4][0];
    $return .= '<iframe width="'.$width.'" height="'.$height.'" src="http://www.youtube.com/embed/' . $video_id . '?wmode=opaque&amp;autohide=1" allowfullscreen></iframe>';  
    return $return;   
}  
...
if (!function_exists('skt_pricing_column')) {
	function skt_pricing_column($atts, $content = null) {
	    ...
	    $html = ""; 
            ...
            if($active == "yes"){
                    $html .= "<div class='active_best_price'><p>". __('Best','advertica') ."</p></div>";
            } 
        $html .= "<ul>";
...
function skt_dynamic_text($atts, $content=null){  
...
add_shortcode('dynamic_text_rotator', 'skt_dynamic_text');
...
function skt_counter($attributes, $content) {
        ...
        $output = 	'<div class="skt-counter span3 fade_in_hide element_fade_in" data-count="'.$attributes['count'].'" data-prefix="'.$attributes['prefix'].'" data-suffix="'.$attributes['suffix'].'" style="color:'.$attributes['color'].';">
						<div class="skt-counter-h">
							...
							<h6 class="skt-counter-title" style="color:'.$attributes['color'].';">'.$attributes['title'].'</h6>
						</div>
					</div>';
        return $output;
}
