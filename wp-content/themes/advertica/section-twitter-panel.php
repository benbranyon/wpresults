<?php global $advertica_shortname; ?>
    <div id="bot-twitter">
            <div class="twitter-row">
                <?php
                $numTweets = sketch_get_option($advertica_shortname."_numb_lat_tw"); // Number of tweets to display.
                $name = sketch_get_option($advertica_shortname."_tw_username");// Username to display tweets from.
                $excludeReplies = true; // Leave out @replies
                $transName = 'list-tweets'; // Name of value in database.
                $cacheTime = sketch_get_option($advertica_shortname."_cachetime"); // Time in minutes between updates.
				...
				$twitter_consumer = sketch_get_option($advertica_shortname."_twitter_consumer");
				$twitter_con_s = sketch_get_option($advertica_shortname."_twitter_con_s");
				$twitter_acc_t = sketch_get_option($advertica_shortname."_twitter_acc_t");
				$twitter_acc_t_s = sketch_get_option($advertica_shortname."_twitter_acc_t_s");
                        ...
                        <?php foreach ($tweets as $t) : ?>
                            <li class="twitter-item">
                                ...
                                <span class="date"><?php echo human_time_diff($t['time'], current_time('timestamp')); ?>
                                    <?php _e('ago','advertica');?>
                                </span>
                            </li>
                        <?php endforeach; ?>

						<?php } else { _e("Please Configure the twitter section in admin : Advertica Options > Home Template Twitter Configuration",'advertica'); } ?>
                    </ul>
                </div>
            </div>
            <div class="nav"></div>
